  ...
  <div className={attachdstyles.join(" ")}>
    ...
    <div className={styles.item}>
        {props.cart.length ? (props.cart.map(cartItem=>(
            <CartItem key={cartItem.id} title={cartItem.title} brand={cartItem.brand} imgURL={cartItem.imgURL} price={cartItem.price}/>
        ))): <div className={styles.message}>Your cart is empty</div>}
        
    </div>
    {props.cart.length ? <button className={styles.redBtn}>Go to cart</button>:null}
  </div>
  </React.Fragment>
);
} 
